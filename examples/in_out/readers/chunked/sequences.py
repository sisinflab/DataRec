...
# --- SEQUENTIAL DATASETS (chunked examples) ---


## --- TABULAR INLINE DATASETS (stream=True) ---
...
print(read_sequence_tabular_inline(os.path.join(output_folder, filename), user_col='user', sequence_col='item', header=0, col_sep='\t', sequence_sep=';', stream=True))
...
print(read_sequence_tabular_inline(os.path.join(output_folder, filename), user_col='user', sequence_col='item', col_sep='\t', sequence_sep=';', cols=['user', 'item'], stream=True))
...
print(read_sequence_tabular_inline(os.path.join(output_folder, filename), user_col='user', sequence_col='item', timestamp_col='timestamp', header=0, col_sep='\t', sequence_sep=';', stream=True))
...
print(read_sequence_tabular_inline(os.path.join(output_folder, filename), user_col='user', sequence_col='item', timestamp_col='timestamp', header=None, col_sep='\t', sequence_sep=';', cols=['user', 'item', 'timestamp'], stream=True))


## --- TABULAR WIDE DATASETS (not chunked in reader; unchanged) ---
...
## --- TABULAR IMPLICIT DATASETS (not chunked in reader; unchanged) ---
...
## --- JSON DATASETS (not chunked in reader; unchanged) ---
...
download_example_by_url(output_folder=output_folder, filename=filename, url='https://raw.githubusercontent.com/sisinflab/DataRecDatasets/refs/heads/main/sequences/json-user-indexed/timestamp/dataset.json')
...
## --- JSON ARRAY DATASETS (not chunked in reader; unchanged) ---
